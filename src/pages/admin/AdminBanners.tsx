  ...
  return (
    <div className="space-y-6 font-['Hind_Siliguri']">
      <div className="bg-white p-4 sm:p-6 rounded-3xl border border-[#E5E0D5] flex items-center justify-between shadow-2xs">
        <div>
          <h2 className="text-base font-bold text-[#123B2A]">ব্যানার ও প্রোমোশন স্লাইডার</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        {banners.map((b) => (
          <div key={b.id} className="bg-white rounded-3xl border border-[#E5E0D5] overflow-hidden shadow-xs">
            <div className="h-40 bg-gray-100 relative">
              {b.imageUrl ? (
                <img src={b.imageUrl} alt={b.altText || b.titleBn} className="w-full h-full object-cover" />
              ) : (
                <div className="w-full h-full flex items-center justify-center bg-[#123B2A] text-[#FFF8EA] font-bold text-sm">
                  {b.titleBn}
                </div>
              )}
              <span className="absolute top-3 left-3 bg-[#D99A2B] text-[#123B2A] text-[10px] font-black px-2 py-0.5 rounded uppercase">
                {b.type}
              </span>
            </div>
            ...
            <div className="p-4 flex flex-col justify-between">
              <div>
                <h3 className="font-bold text-base text-[#123B2A]">{b.titleBn}</h3>
      ...
      {editingBanner && (
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 border border-[#E5E0D5]">
            <h3 className="text-lg font-bold text-[#123B2A] mb-4">ব্যানার এডিট</h3>
            <form onSubmit={handleSave} className="space-y-4 text-sm">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-[#FAF6EE] border border-[#E5E0D5] rounded-xl p-2 text-xs font-bold"
                  >
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-[#FAF6EE] border border-[#E5E0D5] rounded-xl p-2 text-xs"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <label className="flex items-center justify-center gap-2 border-2 border-dashed border-[#E5E0D5] p-3 rounded-xl cursor-pointer hover:border-[#1F6B45]">
                  <Upload className="w-4 h-4 text-gray-400" />
                  <span className="text-xs text-gray-500 font-bold">ইমেজ আপলোড</span>
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="px-5 py-2 rounded-xl bg-[#1F6B45] text-white text-xs font-bold"
                >
                  সংরক্ষণ করুন
                </button>
              </div>
