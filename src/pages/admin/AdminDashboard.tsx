  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white p-5 rounded-3xl border border-[#E5E0D5] shadow-2xs">
          ...
          <div className="text-2xl font-black text-[#123B2A] font-mono">
        ...
        <div className="bg-white p-5 rounded-3xl border border-[#E5E0D5] shadow-2xs">
          ...
          <button
            ...
            className="text-[11px] text-[#1F6B45] font-bold mt-1 hover:underline flex items-center gap-1 cursor-pointer"
          >
      ...
      {lowStockProducts.length > 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-3xl p-5 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h4 className="font-bold text-sm text-[#123B2A]">
          ...
          <button
            ...
            className="bg-[#123B2A] text-white text-xs font-bold px-4 py-2 rounded-xl whitespace-nowrap"
          >
            স্টক আপডেট করুন
          </button>
        </div>
      ...
      <div className="bg-white rounded-3xl p-6 border border-[#E5E0D5] shadow-xs">
        <div className="flex items-center justify-between mb-4 pb-3 border-b border-[#E5E0D5]">
          <h3 className="font-bold text-base text-[#123B2A]">সাম্প্রতিক অর্ডারের তালিকা</h3>
          <button
            onClick={() => onSelectTab('orders')}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs font-['Hind_Siliguri']">
            <thead>
              <tr className="bg-[#FAF6EE] text-gray-500 uppercase tracking-wider font-bold">
            ...
            <tbody className="divide-y divide-[#E5E0D5]/60">
              {orders.slice(0, 6).map((order) => (
                <tr key={order.id} className="hover:bg-[#FAF6EE]/50 transition-colors">
                  <td className="p-3 font-mono font-bold text-[#123B2A]">{order.orderNumber}</td>
                  <td className="p-3 text-gray-500">
                    {new Date(order.createdAt).toLocaleDateString('bn-BD')}
                  </td>
                  <td className="p-3 font-bold text-[#123B2A]">{order.shippingAddress.fullName}</td>
                  ...
                  <td className="p-3 font-black text-[#1F6B45]">
                    ৳{order.grandTotal.toLocaleString()}
                  </td>
                  <td className="p-3">
                    <span className="bg-[#1F6B45]/10 text-[#1F6B45] px-2.5 py-1 rounded-full text-[11px] font-bold">
                      {order.orderStatus}
                    </span>
                  </td>
                </tr>
