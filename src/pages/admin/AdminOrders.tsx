    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-4 sm:p-6 rounded-3xl border border-[#E5E0D5] flex flex-col sm:flex-row items-center justify-between gap-4 shadow-2xs">
        ...
        <div className="flex items-center gap-2 self-start sm:self-auto">
          ...
          <select
            ...
            className="bg-[#F5FBF2] border border-[#DCECD5] text-xs font-bold rounded-xl py-2 px-3 outline-none text-[#004F18]"
          >
          ...
          <table className="w-full text-left text-xs font-['Hind_Siliguri']">
            ...
            <tbody className="divide-y divide-[#DCECD5]">
              {filteredOrders.map((order) => (
                <tr key={order.id} className="hover:bg-[#F5FBF2]/50 transition-colors">
                  <td className="p-4 font-mono font-bold text-[#004F18]">{order.orderNumber}</td>
                  <td className="p-4 text-gray-500">
                    {new Date(order.createdAt).toLocaleDateString('bn-BD')}
                  </td>
                  <td className="p-4">
                    <div className="font-bold text-[#102B16]">{order.shippingAddress.fullName}</div>
                  ...
                  <td className="p-4 font-black text-[#004F18]">
                  ...
                  <td className="p-4">
                    <select
                      ...
                      className="bg-[#E8F8D8] border border-[#5EB809]/40 rounded-lg px-2 py-1 text-xs font-bold text-[#004F18] outline-none cursor-pointer"
                    >
                      {STATUS_LIST.map((st) => (
                        <option key={st} value={st}>
                          {STATUS_BN[st]}
                  ...
                  <td className="p-4 text-right">
                    <button
                      ...
                      className="bg-[#004F18] text-white p-2 rounded-xl hover:bg-[#063B14] transition-colors cursor-pointer"
                      title="বিস্তারিত চালান দেখুন"
                    >
                      <Eye className="w-4 h-4 text-[#5EB809]" />
                    </button>
                  </td>
                </tr>
              ))}
      ...
      {selectedOrder && (
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-2xl w-full p-6 sm:p-8 max-h-[90vh] overflow-y-auto border border-[#DCECD5] font-['Hind_Siliguri'] shadow-2xl">
            <div className="flex justify-between items-center pb-4 border-b border-[#DCECD5] mb-6">
              <div>
                ...
                <h3 className="text-xl font-black text-[#004F18] font-mono">
                  {selectedOrder.orderNumber}
                </h3>
              </div>
              <button
                onClick={() => setSelectedOrder(null)}
                className="text-gray-400 hover:text-gray-600 p-2 text-lg font-bold cursor-pointer"
              >
                ✕
              </button>
            </div>
            ...
            <div className="space-y-6 text-sm">
              {/* Shipping info */}
              <div className="grid grid-cols-2 gap-4 bg-[#F5FBF2] p-4 rounded-2xl border border-[#DCECD5]">
                <div>
                  <span className="text-xs font-bold text-gray-400 block uppercase">প্রাপক</span>
                  <p className="font-bold text-[#102B16]">{selectedOrder.shippingAddress.fullName}</p>
                ...
                <div>
                  ...
                  <p className="text-xs font-bold text-[#004F18]">{selectedOrder.shippingAddress.district}</p>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="border border-[#DCECD5] rounded-2xl overflow-hidden">
                  <table className="w-full text-left text-xs">
                    <thead className="bg-[#F5FBF2] font-bold text-gray-600 border-b border-[#DCECD5]">
                      <tr>
                    ...
                    <tbody className="divide-y divide-[#DCECD5]">
                      {selectedOrder.items.map((it, idx) => (
                        <tr key={idx}>
                          <td className="p-3 font-bold text-[#102B16]">{it.nameBn}</td>
                          <td className="p-3">৳{it.price}</td>
                          <td className="p-3">{it.quantity}</td>
                          <td className="p-3 text-right font-black text-[#004F18]">
              ...
              {/* Summary Calculation */}
              <div className="space-y-1.5 text-xs text-right border-t border-[#DCECD5] pt-4">
                ...
                <div className="flex justify-between text-base font-black text-[#004F18] pt-2 border-t border-[#DCECD5]">
                  ...
                  <span className="text-[#004F18]">৳{selectedOrder.grandTotal.toLocaleString()}</span>
                </div>
              </div>

              {/* Action bar */}
              <div className="flex justify-between items-center pt-4 border-t border-[#DCECD5]">
                <button
                ...
                >
                  <Printer className="w-4 h-4 text-[#004F18]" />
                  <span>চালান প্রিন্ট করুন</span>
                </button>
                <button
                  onClick={() => setSelectedOrder(null)}
                  className="bg-[#004F18] hover:bg-[#063B14] text-white px-5 py-2 rounded-xl text-xs font-bold cursor-pointer transition-colors"
                >
                  বন্ধ করুন
                </button>
              </div>
